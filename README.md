...
У вас вся логика все еще синхронизирована. Такого быть не должно. Это бесполезно. Синхронизироваться надо над тем, к чему многие потоки имеют доступ одновременно, к разделяемым объектам. Вот тут есть разделяемые объекты (стр. 45-49) Нет, тут все переменные локальные, они создаются в рамках одного потока.
## Описание
